...
const int32 UMyBlueprintFunctionLibraryTest::FACTEUR_DISTANCE = 15000;
...
FVector UMyBlueprintFunctionLibraryTest::GetInitialeVelocite(const TArray<int32>& Time, int32 index)
{
    ...
    float temps = 86400.0f; // nombre de seconde dans une journee
...
float  UMyBlueprintFunctionLibraryTest::GetSize(int32 index)
{
    if (index >= 0 && index < NOM_PLANETE.Num())
    {
        return -1;
